...
header("Content-Disposition: attachment; filename=frecuenciasParametrosVariables.xls");
...
$frecuencias = new frecuencias_parametros_variables();
$resFrecuencias = $frecuencias->frecuenciasParametrosVariablesPorAreayMaquina( $_GET[ 'codigoPlanta' ] );
$resHorasFrecuencias = $frecuencias->frecuenciasParametrosVariablesHorasPorAreayMaquina( $_GET[ 'codigoPlanta' ] );

foreach ( $resHorasFrecuencias as $registro1 ) {
  //codigoArea, codigoMaquina, nombreVariable
  $vectorResultado[ $registro1[ 0 ] ][ $registro1[ 2 ] ][ $registro1[ 5 ] ][date("H:i", strtotime($registro1[ 8 ]))][$registro1[9]] = 'x';
}
?>
<meta charset="utf-8">
<h3 align="center">Variable críticas - Parámetros Variables</h3>
<br>
    ...
    <tbody class="buscar">
      ...
      <tr>
        ...
        <?php $ti = 0; for ( $i = $horaInicial; $i <= $horaFinal; $i = date( "Y-m-d H:i", strtotime( $i . " + 1 hour" ) ) ) { ?>
          ...
          <?php if(isset($vectorResultado[$registro[0]][$registro[2]][$registro[4]][date("H:i", strtotime($i))][$registro[5]])){ ?>
            <td><?php echo $vectorResultado[$registro[0]][$registro[2]][$registro[4]][date("H:i", strtotime($i))][$registro[5]]; ?></td>
          <?php }else{ ?>
            <td></td>
          <?php } ?>
        <?php if($ti >= 24){ exit(); } $ti++; } ?>
