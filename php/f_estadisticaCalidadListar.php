...
if ( $_POST[ 'turnos' ] != "-1" ) {
  $HoraInicial = date( "Y-m-d H:i", strtotime($_POST['fechaInicial']." ".$tur->getTur_HoraInicio() ) );
  $HoraFinal = date( "Y-m-d H:i", strtotime($_POST['fechaFinal']." ".$tur->getTur_HoraFin() . " - 1 hour" ) );
  if ( $HoraInicial > $HoraFinal ) {
    $HoraFinal = date( "Y-m-d H:i", strtotime( $HoraFinal . " + 1 days" ) );
  }
}else{
  $HoraInicial = date( "Y-m-d H:i", strtotime( $_POST[ 'fechaInicial' ]. " " . $_POST['horaI']  ) );
  $HoraFinal = date( "Y-m-d H:i", strtotime( $_POST[ 'fechaFinal' ]. " " . $_POST['horaF'] ) );
}
...
if ( $HoraInicialValTEsp > $HoraFinalValTEsp ) {
  $fechaInicioFiltro = strtotime($_POST['fechaInicial']);
  $fechaFinFiltro = strtotime($_POST['fechaFinal']);
}else{
  if(date( "Y-m-d", strtotime( $_POST['fechaInicial'] . " + 1 days" )) == date( "Y-m-d", strtotime( $_POST['fechaFinal']))){
    $fechaInicioFiltro = strtotime($_POST['fechaInicial']);
    $fechaFinFiltro = strtotime($_POST['fechaInicial']);
  }else{
    $fechaInicioFiltro = strtotime($_POST['fechaInicial']);
    $fechaFinFiltro = strtotime($_POST['fechaFinal']);
  }
}
for($i=$fechaInicioFiltro; $i<=$fechaFinFiltro; $i+=86400){
    array_push($fechasFiltro, date("Y-m-d", $i));
}

$porcentajeCalidad = new porcentajes_calidad();
$resPorcentajeCalidad = $porcentajeCalidad->listarPorcentajesCalidadSupervisorInforme( $vecReferenciaConsulta, $vecFormato, $vecFamilia, $vecColor, date( "H:i", strtotime( $HoraInicial ) ), date( "H:i", strtotime( $HoraFinal ) ), $_POST[ 'turnos' ], $fechaIniT3, $fechaFinT3, $HoraInicialRespT, $HoraFinalRespT, $HoraInicialRespT2, $HoraFinalRespT2, $valEspTurnoR, $resTurn[0], $resTurn[1] );
...
//  echo "fecha ".$registro2[ 7 ]." hora ".$registro2[ 14 ]."valor ".$vecValor[ $agrupacion ][ $registro2[ 13 ] ][ $registro2[ 9 ] ][ $registro2[ 10 ] ][ $registro2[ 11 ] ]."<br>";
  ...
  $vecAgrupacionCod[ $agrupacion ] = $_POST[ 'agrupacion' ];
...
$resRes = $resC->listarRespuestasCalidadTodasHorasTSInforme( $vecReferenciaConsulta, $vecFormato, $vecFamilia, $vecColor, $_POST[ 'area' ], $fechaIniT3, $fechaFinT3, $HoraInicialRespT, $HoraFinalRespT, $HoraInicialRespT2, $HoraFinalRespT2, $valEspTurnoR, $resTurn[0], $resTurn[1] );
...
<div class="table-responsive">
  <table id="tbl_porcentajeCalidadPrincipal" border="1px" class="table tableEstrecha table-hover table-bordered table-striped">
    <thead>
      <tr class="encabezadoTab letra14 vertical">
    ...
    <tr class="encabezadoTab">
      <?php if($_POST['agrupacion'] != "4"){ ?>
      <td colspan="2" align="center">TOTAL M<sup>2</sup>:</td>
      <?php }else{ ?>
      ...
      <?php } ?>
      <td align="center"><?php echo number_format($sumaTotalMetros, 0, ".", ""); ?></td>
      <?php foreach($vecAgrupacionEncab as $registro5){ ?>
          <td></td>
      <?php } ?>
    </tr> 
...
$(function () {
    $('#Grafico_EstadisticaCalidadMetros').highcharts({
        chart: {
            type: 'column'
        },
        title: {
            text: 'Detalle Calidad - Metros cuadrados'
        },
       exporting: { enabled: false },
        xAxis: {
            categories: [
              <?php $cantPar = 0;
                foreach($vecAgrupacionEncab as $registro5){
                    if($cantPar > 0){
                      echo ",";
                    }
                  switch ( $registro5 ) {
        ...
        series: [
          ...
          foreach($vecReferenciaConsulta as $registro9){
            foreach($vecAgrupacion as $registro3){
              if(isset($vecAgrupacion2[$registro3][$vecFormato[$registro9]][$vecFamilia[$registro9]][$vecColor[$registro9]])){
                if($ConNa > 0){
                  echo ",";
                }
              ?>
              { name: '<?php if($_POST['agrupacion'] == "4"){ $VarNomGra = $vecFormatoNombre[$registro9]." ".$vecFamilia[$registro9]." ".$vecColor[$registro9]; }else{ $VarNomGra = $registro3; } echo $VarNomGra."(".$vecFamilia[$registro9].")"; ?>', data: [
                ...
                foreach($vecAgrupacionEncab as $registro5){
                  ...
                  if(isset($MetrosCalidad[$registro3][$registro5][$vecFormato[$registro9]][$vecFamilia[$registro9]][$vecColor[$registro9]])){
                    $ValGra = $MetrosCalidad[$registro3][$registro5][$vecFormato[$registro9]][$vecFamilia[$registro9]][$vecColor[$registro9]];
                  }else{
                    $ValGra = 0;
                  }
...
$(function () {
    $('#Grafico_EstadisticaCalidad').highcharts({
        chart: {
            type: 'column'
        },
        title: {
            text: 'Detalle Calidad - Porcentaje'
        },
       exporting: { enabled: false },
        xAxis: {
            categories: [
        ...
        series: [
          ...
          foreach($vecReferenciaConsulta as $registro9){
            foreach($vecAgrupacion as $registro3){
              if(isset($vecAgrupacion2[$registro3][$vecFormato[$registro9]][$vecFamilia[$registro9]][$vecColor[$registro9]])&& !is_nan($calidadRespuesta[$registro3][$registro5][$vecFormato[$registro9]][$vecFamilia[$registro9]][$vecColor[$registro9]])){
                if($ConNa > 0){
                  echo ",";
                }
              ?>
              { name: '<?php if($_POST['agrupacion'] == "4"){ $VarNomGra = $vecFormatoNombre[$registro9]." ".$vecFamilia[$registro9]." ".$vecColor[$registro9]; }else{ $VarNomGra = $registro3; } echo $VarNomGra."(".$vecFamilia[$registro9].")"; ?>', data: [
                ...
                if(isset($calidadRespuesta[$registro3][$registro5][$vecFormato[$registro9]][$vecFamilia[$registro9]][$vecColor[$registro9]])){
                    $ValGra = number_format($calidadRespuesta[$registro3][$registro5][$vecFormato[$registro9]][$vecFamilia[$registro9]][$vecColor[$registro9]], 2, ".", "");
                  }else{
                    $ValGra = 0;
                  }
              ...
              <table id="tbl_porcentajeCalidadSegunda" border="1px" class="table tableEstrecha table-hover table-bordered table-striped">
                <thead>
                  <tr class="encabezadoTab letra14">
                    ...
                    <th align="center" class="text-center">TOTAL m<sup>2</sup></th>
                ...
                <tbody class="buscar">
                  ...
                  <tr class="encabezadoTab">
                    <td>Total:</td>
                    <td align="center"><?php echo number_format($TotalDefectosSegunda, 0, ".", ""); ?></td>
                    <td align="center"><?php  echo number_format($totalParticipacionSegunda, 2, ".", "")."%"; ?></td>
                  </tr>
                </tbody>
              </table>
            </div>
  ...
  $(document).ready(function(e){ 
    $('#Grafico_EstadisticaDefectologiaSegunda').highcharts({
        ...
        series: [{
            ...
            data: [
             <?php $cantPar = 0;
                 for($i=0; $i<$cantDefectos; $i++){ 
                    if($cantPar > 0){
                      echo ",";
                    }
                   
                   echo $resultadoTotalParticipacion[$i];
                   $cantPar++;
               } 
            ?>
            ]
          ...
          data: [
           <?php $cantPar = 0;
              for($i=0; $i<$cantDefectos; $i++){ 
                   ...
                   $acumuladoGrafico += $resultadoTotalParticipacion[$i];
                 //Pareto
                  $participacionRetalGrafPareto = $resultadoTotalMetros[$i] / $TotalDefectosSegunda;
                  $participacionRetalGrafParetoSum += $participacionRetalGrafPareto;

                  echo "['Pareto:".number_format($participacionRetalGrafParetoSum*100, 0, ".", "")."%',".number_format($acumuladoGrafico, 2, ".", "")."]";
            ...
            <div class="table-responsive" id="imp_tabla">
              <table id="tbl_porcentajeCalidadRotura" border="1px" class="table tableEstrecha table-hover table-bordered table-striped">
                ...
                <tbody class="buscar">  
                  ...
                  <tr class="encabezadoTab">
                    <td>Total:</td>
                    <td align="center"><?php echo number_format($TotalDefectosRetal, 0, ".", ""); ?></td>
                    <td align="center"><?php  echo number_format($totalParticipacionRetal, 2, ".", "")."%"; ?></td>
                  </tr>
                </tbody>
              </table>
            </div>
  ...
  $(document).ready(function(e){
    $('#Grafico_EstadisticaDefectologiaRotura').highcharts({
        ...
        series: [{
            ...
            data: [
             <?php $cantPar = 0;
                for($i=0; $i<$cantDefectosRetal; $i++){ 
                    if($cantPar > 0){
                      echo ",";
                    }
                   
                   echo $resultadoTotalParticipacionRetal[$i];
                   $cantPar++;
               }
            ?>
            ]
          ...
          data: [
           <?php $cantPar = 0;
              for($i=0; $i<$cantDefectosRetal; $i++){ 
                   ...
                   $acumuladoGraficoRetal += $resultadoTotalParticipacionRetal[$i];
                
                 //Pareto
                $participacionRetalGrafPareto2 = $resultadoTotalMetrosRetal[$i] / $TotalDefectosRetal;
                $participacionRetalGrafParetoSum2 += $participacionRetalGrafPareto2;

                echo "['Pareto:".number_format($participacionRetalGrafParetoSum2*100, 0, ".", "")."%',".number_format($acumuladoGraficoRetal, 2, ".", "")."]";
                $cantPar++;
               } 
          ?>
          ]
