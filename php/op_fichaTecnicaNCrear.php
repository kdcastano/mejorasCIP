...
$obsFT = new ft_pdf_observaciones();
$obsFT2 = new ft_pdf_observaciones();

$det = new detalle_ficha_tecnica();
$resDet = $det->listarDTFTodosNClonarSoloMaquinasActivas( $_POST[ 'codigoFT' ] );
...
if ( $resultado[ 'resultado' ] ) {
  ...
  if ( $_POST[ 'clonar' ] == 1 ) {
    $resFic = $fic->listarUltimoIdFTN( $_SESSION[ 'CP_Usuario' ] );
    
    $resObsFT = $obsFT->listarObservacionTipo($_POST[ 'codigoFT' ]);
    
    //Agregar observaciones 
    foreach($resObsFT as $registro2){
      $obsFT2->setFicT_Codigo($resFic[ 0 ]);
      $obsFT2->setFt_pdf_Tipo($registro2[2]);
      $obsFT2->setFt_pdf_Observaciones($registro2[3]);
      $obsFT2->setFt_pdf_FechaHoraCrea( $fecha . ' ' . $hora );
      $obsFT2->setFt_pdf_UsuarioCrea($_SESSION[ 'CP_Usuario' ]);
      $obsFT2->setFt_pdf_Estado('1');
      $resultado[ 'resultado2' ] = $obsFT2->insertar();
    }

    foreach ( $resDet as $registro ) {
      $det->setFicT_Codigo( $resFic[ 0 ] );
      $det->setMaq_Codigo( $registro[ 2 ] );
      $det->setAgrVCon_Codigo( $registro[ 3 ] );
      $det->setAgrMCon_Codigo( $registro[ 4 ] );
      $det->setDetFT_Tipo( $registro[ 5 ] );
      $det->setDetFT_UnidadMedida( $registro[ 6 ] );
      if ( $registro[ 5 ] == 1 ) {
        $det->setDetFT_ValorControlTexto( $registro[ 8 ] );
        $det->setDetFT_ValorControl(NULL);
      } else {
        $det->setDetFT_ValorControl( $registro[ 7 ] );
        $det->setDetFT_ValorControlTexto(NULL);
      }
      $det->setDetFT_ValorTolerancia( $registro[ 9 ] );
      $det->setDetFT_Operador( $registro[ 10 ] );
      $det->setDetFT_TomaVariable( $registro[ 11 ] );
  ...
  $resultado[ 'mensaje2' ] = $obsFT2->imprimirError();
}
