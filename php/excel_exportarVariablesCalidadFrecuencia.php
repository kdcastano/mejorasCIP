...
header("Content-Disposition: attachment; filename=frecuenciasVariablesCalidad.xls");
...
$frecuencias = new frecuencias_calidad();
$resFrecuencias = $frecuencias->frecuenciasVariablesCalidadPorAreayMaquina( $_GET[ 'codigoPlanta' ] );
$resHorasFrecuencias = $frecuencias->frecuenciasVariablesCalidadHorasPorAreayMaquina( $_GET[ 'codigoPlanta' ] );

foreach ( $resHorasFrecuencias as $registro1 ) {
  //codigoArea, calidadNombre
  $vectorResultado[ $registro1[ 0 ] ][ $registro1[ 3 ] ][date("H:i", strtotime($registro1[ 6 ]))] = 'x';
}
?>
<meta charset="utf-8">
<h3 align="center">Variable críticas - Calidad</h3>
<br>
    ...
    <tbody class="buscar">
      ...
      <tr>
        ...
        <?php $ti = 0; for ( $i = $horaInicial; $i <= $horaFinal; $i = date( "Y-m-d H:i", strtotime( $i . " + 1 hour" ) ) ) { ?>
          ...
          <?php if(isset($vectorResultado[$registro[0]][$registro[2]][date("H:i", strtotime($i))])){ ?>
            <td><?php echo $vectorResultado[$registro[0]][$registro[2]][date("H:i", strtotime($i))]; ?></td>
          <?php }else{ ?>
            <td></td>
          <?php } ?>
        <?php if($ti >= 24){ exit(); } $ti++; } ?>
